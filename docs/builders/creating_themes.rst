...
:xref:`MJML` allows marketers to create and maintain beautiful and responsive Themes for MailFly easily.
...
Since :xref:`MailFly 3` a new line defines the compatible Builders:
...
At present, MailFly won't process the ``<mj-head>`` tags. None of the ``<mj-attribuites>`` run, so you have to do all styling inline.
...
At present, MailFly processes most ``<mj-body>`` components.
