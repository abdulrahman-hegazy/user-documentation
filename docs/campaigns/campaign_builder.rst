...
The actions that MailFly offers in a Campaign include:
...
.. list-table:: Campaign Actions in MailFly
   ...
   * - **Send Email to User** 
     - Sends an Email to an entity other than the Contact. This may be a MailFly User, the Contact's owner, or non-Users. Emails sent using this action don't generate any statistics for Contacts or Emails.
...
.. image:: images/campaign-decisions.gif
    :width: 600
    :alt: Screenshot showing Campaign decisions available in MailFly

Here are the decisions that MailFly offers in the Campaign Builder:

.. list-table:: Decision Actions in MailFly
   :header-rows: 1
   :widths: 5, 95
   ...
   * - **Submits Form**    
     - Set options to track whether the Contact has submitted any MailFly Forms. You can also limit this decision to track specific Forms.
   * - **Visits a page**    
     - Specify one or multiple pages you want the Contact to visit. Can be MailFly Landing Pages or pages on your website.
...
.. list-table:: Email-related Decision Actions in MailFly
   :header-rows: 1
   :widths: 5, 95
...
Here are the different conditions that MailFly offers in the Campaign Builder:

.. list-table:: Condition Actions in MailFly
   :header-rows: 1
   :widths: 5, 95
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
MailFly respects delays set on the condition itself before passing down to a delay on any connected action. For example, if you are coming from a negative path on 'Opens Email', you can set a condition of 'has active notification' with a relative date of 1 day, followed by 'Send Email' on the negative path with a relative date of 2 days. MailFly checks after 1 day if there is an active notification and if there isn't, schedules the Email for two days later.
...
MailFly evaluates Campaign conditions immediately, therefore if the date in the field matches the condition, MailFly executes then the positive action. If the date doesn't match, MailFly executes the negative action. The Contact doesn't wait for the condition to be TRUE.
...
Actions and Decisions in MailFly require a :doc:`cron job</configuration/cron_jobs>` which executes the following command at the desired interval:
