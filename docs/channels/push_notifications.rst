...
Mobile notifications integrate your iOS and Android app with :xref:`OneSignal`. Using your own OneSignal account, you can now push a notification to your Contacts's mobile device - with their permission. Enable the Plugin in MailFly's Plugin manager to see Mobile Notifications listed under Channels in the menu.
...
.. code-block:: javascript
    ...
    OneSignal.idsAvailable({(_ userId, _ pushToken) in
        ...
        self.pushUserDataToMailFly(userData, "https://MailFly.example.com/notification/appcallback")
    });

For ease of use, we've created the following struct and func for sending user data to MailFly. Create this struct in your app, and import it where appropriate.
...
pushUserDataToMailFly func
~~~~~~~~~~~~~~~~~~~~~~~~~

This is a basic function for pushing the UserData struct to your MailFly installation. It will push the User data, and then display the response from MailFly as an app alert. Modify to meet the needs of your app.

.. code-block:: javascript

    func pushUserDataToMailFly(_ userData: UserData, _ url: String) {
...
In addition to the UserData that gets pushed to MailFly, you can push open / interaction stats to MailFly by sending the UserData struct, with an appended stat JSON key.
