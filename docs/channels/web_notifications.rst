...
Web Notifications can be an extremely powerful tool for the marketer. MailFly integrated with :xref:`OneSignal` which allows you to push information to a Contact as they browse through web resources (MailFly Landing Pages, or with some configuration (see :xref:`OneSignal web push`), on your own website), giving you one more Channel that you can use to build a relationship with them.

Web notifications in MailFly use OneSignal by default. Using your own OneSignal account, you can now push a notification to your Contacts's browser - with their permission. Enable the OneSignal Plugin in MailFly's Configuration to see Web Notifications listed under Channels in the menu.
...
#. Download the SDK files from the next screen, and upload them to the root of your MailFly installation - this must be accessible at `https://MailFly.example.com/OneSignalSDKWorker.js`.
