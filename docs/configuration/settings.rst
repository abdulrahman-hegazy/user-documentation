MailFly configuration settings
#############################
...
.. image:: images/general-settings.png
  :width: 600
  :alt: Screenshot showing General Settings Configuration in MailFly

* **Site URL** - This is where MailFly is physically installed. Set the URL for this site here. Cron jobs needs this to correctly determine absolute URLs when generating links for Emails, etc. It 's also called MailFly's 'base URL'.

* **MailFly's root URL** - When a User signs in to their MailFly instance, they go to ``MailFly.example.com``. However, that Landing Page is also accessible to the public. If a Contact visits that address, they see the MailFly login page for that instance. 

  To brand that Landing Page, create a MailFly Landing Page that you'd want to greet any Contacts who visit your root ``URL``. Once you've done that, Users can sign in into MailFly by visiting ``MailFly.example.com/s/login``.

* **404 page** - Select the Landing Page that you want to use as the 404 Landing Page. If you don't want to use MailFly's default 404 error page, create a custom Landing Page and select that page here. If you don't select any page, MailFly uses the default error page.
...
.. image:: images/system-default-settings.png
  :width: 600
  :alt: Screenshot showing System defaults Settings Configuration in MailFly
...
* **Default language** - The initial language assigned to Users. Individual Users may select their own settings. MailFly uses ``English - United States`` by default.

* **Cached data timeout (minutes)** - MailFly caches data to speed up page loads. Update this setting to change how long MailFly caches the data for. MailFly uses ``10 minutes`` as the default.

* **Date Range Filter Default** - Sets the default for how far back from the current date MailFly looks for data in Reports including Campaign and Email snapshots charts on the item page. This setting allows you to control the default for how far back from the current date MailFly looks for data. If you've changed the setting on a Report, MailFly uses what you've entered. MailFly's default value is ``1 Month``.
...
Cross-Origin Resource Sharing (CORS) enables data to pass between your website and MailFly.

.. image:: images/cors-settings.png
  :width: 600
  :alt: Screenshot showing CORS Settings Configuration in MailFly

* **Restrict Domains** - When set to No, any web page can pass information to MailFly. Select Yes to limit communication with your MailFly instance to websites listed in Valid Domains strongly recommended.

* **Valid Domains** - A list of domains allowed to communicate with your MailFly instance. In the text box, list the exact URL of the top level domain you want to allow, one per line. For example: ``http://www.example.com`` tracks any activity on non-secure example.com pages, but ``https://www.example.com`` won't because this is only tracking on a secure (``https://``) website.
...
.. image:: images/miscellaneous-settings.png
  :width: 600
  :alt: Screenshot showing Miscellaneous Settings Configuration in MailFly

* **Trusted hosts** - To explicitly allow the hosts that can send requests to MailFly. You can use regular expression and separate multiple hosts with a comma. i.e ``.*\.?example.com$``. If left empty, MailFly will respond to all hosts.
  
* **Trusted proxies** - To configure the IP addresses that should be trusted as proxies. This setting is mandatory when using MailFly behind an SSL terminating proxy. Separate multiple IP addresses by a comma. i.e ``127.0.0.1, 10.0.0.0/8, fc00::/7``

* **IP lookup service** - By default, MailFly uses :xref:`MaxMind's` database to identify the city of a website visitor, based on the location of the Internet Service Provider (ISP) for their IP address.

* **IP lookup service authentication** - To use any IP lookup service which requires authentication, enter your credentials.
  
* **List of IPs not to track Contacts with** - To turn off tracking for particular IP addresses, enter the addresses, one per line. MailFly doesn't recommend adding your office IP address. If you list your internal IP address, MailFly won't track clicks, page hits, etc., from that IP, **including when you are testing functionality**.

* **List of known Bots** - MailFly has the feature to identify and turn-off tracking for several known bots. To track activity from those bots, remove them from this list. To turn off tracking for other bots, add them here (one per line).
...
* **Item max lock time** - When a User edits a Campaign, Email, Landing Page, etc., MailFly locks the item to prevent simultaneous edits by other Users. When the initial User saves and closes or cancels out, the item may remain locked for this period of time. The default is ``0 seconds``.
...
.. image:: images/update-settings.png
  :width: 600
  :alt: Screenshot showing Update Settings Configuration in MailFly
...
* **Update MailFly through Composer [BETA]**  - Set to 'Yes' if you update MailFly through Composer. This is a prerequisite if you want to install and update Plugins through the Marketplace. This becomes the default installation and update method in MailFly 5.
...
.. image:: images/theme-settings.png
  :width: 600
  :alt: Screenshot showing Theme Settings Configuration in MailFly

* **Default Theme** - Applies a Theme to any Form which doesn't have a Theme already applied. If you don't have a Landing Page for your Preference Center, but have preference settings turned on in Email settings, MailFly creates a default Preference Center page using the Form styling from the Theme selected here.
...
.. image:: images/api-settings.png
  :width: 600
  :alt: Screenshot showing API Settings Configuration in MailFly

Full API documentation is available :xref:`MailFly developer API`.

* **API enabled** - Select Yes to pass data in and out of MailFly through the API.

* **Enable http basic auth?** - Enables basic authentication for MailFly's API. It's recommended to only use this with secure sites (https).
...
.. image:: images/assets-settings.png
  :width: 600
  :alt: Screenshot showing Assets Settings Configuration in MailFly
...
.. image:: images/campaign-settings.png
  :width: 600
  :alt: Screenshot showing Campaign Settings Configuration in MailFly

* **Wait time before retrying a failed action** - If for any reason a Campaign action doesn't execute, this is the length of time MailFly waits before trying again.
...
As MailFly uses the :xref:`Symfony Mailer` library since v5, it supports all Symfony Mailer core plugins out of the box. Transports for other Email services might be found on GitHub or Packagist.

SMTP transport
--------------

The SMTP transport is the default transport used for sending Emails with MailFly. It's configured in the MailFly configuration under the Email Settings tab. The configuration is the same as in the :xref:`Symfony Mailer` documentation.

MailFly now uses a specific way of providing the connection details for Email transports to interpret, known as a Data Source Name, or DSN. This is the example Data Source Name configuration mentioned in the :xref:`Symfony Mailer` documentation for SMTP:

.. code-block:: shell
    
    smtp://user:pass@smtp.example.com:port

MailFly creates this automatically from the values entered in the Email configuration:
...
.. note::
  Use the MailFly's global configuration to paste in the DSN information, especially the API keys and passwords. The values must be URL-encoded, and the configuration form does that for you. If you are pasting DSN settings directly into the config/local.php file, you must URL-encode the values yourself.
...
.. warning::
  Installing Symfony Transports is possible when you've :doc:`installed MailFly via Composer </getting_started/how_to_install_MailFly.rst>`. 
...
After that, you can configure the transport in the MailFly configuration. The example DSN is again mentioned in the :xref:`Symfony Mailer` documentation along with other transports. In the example of using the Sendgrid API, the DSN looks like this:

.. code-block:: shell
    
    sendgrid+api://KEY@default

This is how it would be set up in MailFly's Email configuration:

  .. image:: images/sendgrid-api-dsn.png
    :width: 400
    :alt: Sendgrid API DSN example

To replace the SendGrid API key, add it to the relevant field in the Email configuration and save. MailFly now uses the SendGrid API to send Emails. 

.. warning::
  It's a nice perk that MailFly can use any transport provided by Symfony Mailer. However, be aware that such transports (from Symfony) don't support batch sending, even via API. They only send one email per request, as opposed to a thousand emails per request as is the case with some MailFly transports, which can make them slow at scale. They also don't support transport callback handling used for bounce management. If you plan to send larger volumes of Emails or need to use features which require callback handling, please consider using Email transports built specifically for such use. These plugins are available in the :doc:`MailFly Marketplace </marketplace/marketplace.rst>`.
...
This is the default means of delivery. As soon as an action in MailFly triggers an Email to send, it's sent immediately. If you expect to send a large number of Emails, you should use the queue. Sending Email immediately may slow the response time of MailFly if using a remote mail service, since MailFly has to establish a connection with that service before sending the mail. Also attempting to send large batches of Emails at once may hit your server's resource limits or Email sending limits if on a shared host.

Queued delivery
---------------

MailFly works most effectively with high send volumes if you use the queued delivery method. MailFly stores the Email in the configured spool directory until the execution of the command to process the queue. Set up a :doc:`cron job </configuration/cron_jobs>` at the desired interval to run the command:

.. code-block:: shell
    
    php /path/to/MailFly/bin/console messenger:consume email_transport

Some hosts may have limits on the number of Emails sent during a specified time frame and/or limit the execution time of a script. If that's the case for you, or if you just want to moderate batch processing, you can configure batch numbers and time limits in MailFly's Configuration. See the :doc:`cron job documentation </configuration/cron_jobs>` for more specifics.
...
.. image:: images/mail-send-settings.png
  :width: 600
  :alt: Screenshot showing Mail Send Settings Configuration in MailFly
...
* **Mailer is owner** - If Contacts in MailFly have owners, select Yes to use the Contact owner as the sender of Emails to any Contacts they're listed as the owner for. 
...
.. image:: images/monitored-settings.png
  :width: 600
  :alt: Screenshot showing Monitored Settings Configuration in MailFly
...
* **Contact Replies** - Similar to the monitored inbox for bounces, this is the inbox MailFly checks for Contact replies. Using :ref:`Replies to Email<email settings>` decisions in any Campaign requires configuration. With ``Use custom connection settings?`` set to ``no``, MailFly checks the default mailbox. If set to ``yes``, you may track a different mailbox for replies.
...
.. image:: images/message-settings.png
  :width: 600
  :alt: Screenshot showing Message Settings Configuration in MailFly
...
.. image:: images/unsubscribe-settings.png
  :width: 600
  :alt: Screenshot showing Unsubscribe Settings Configuration in MailFly
...
* **Show Contact preference settings** - Select **Yes** to direct the unsubscribe link to your configured Preference enter. If you haven't created a Preference Center, MailFly creates a default page based on the next 5 settings. The created page uses the default Theme for styling.
...
* **Show pause Contact preferences** - Select **Yes** to allow Contacts to turn-off messages from your MailFly account to their Email address for a specified date range. This action isn't a full unsubscribe action, and at the end of the date range, In this case, it sends the message again after the date range ends, as this isn't a full unsubscribe action.
...
* **Show Contact's preferred Channel option** - If you have multiple Channels available within your MailFly instance. For example; Email, ``SMS``, mobile push, web notifications, etc., Contacts can choose their preferred Channel. This can be useful if you are using the Marketing Messages feature of MailFly. More information about the Preference Center is available :doc:`here</contacts/preference_center>`.
...
MailFly automatically tags each Email with a tracking pixel image. This allows MailFly to track when a Contact opens the Email and execute actions accordingly. Note that there are limitations to this technology - the Contact's Email client supporting HTML and auto-loading of images, and not blocking the loading of pixels. If the Email client doesn't load the image, there's no way for MailFly to know the opened status of the Email.

By default, MailFly adds the tracking pixel image at the end of the message, just before the ``</body>`` tag. If needed, one could use the ``{tracking_pixel}`` variable within the body content token to have it placed elsewhere. Beware that you shouldn't insert this directly after the opening ``<body>`` because this prevents correct display of pre-header text on some Email clients.
...
MailFly tracks clicks of each link in an Email, with the stats displayed at the bottom of each Email detail view under the Click Counts tab.
...
.. image:: images/form-settings.png
  :width: 600
  :alt: Screenshot showing Form Settings Configuration in MailFly
...
.. image:: images/contact-merge-settings.png
  :width: 600
  :alt: Screenshot showing Contact Merge Settings Configuration in MailFly
...
.. image:: images/contact-list-settings.png
  :width: 600
  :alt: Screenshot showing Contact List Settings Configuration in MailFly

* **Columns** - Select from the left which fields appear on the Contact lists (when you go to Contacts in the MailFly and view the list).
...
.. image:: images/import-settings.png
  :width: 600
  :alt: Screenshot showing Import Settings Configuration in MailFly
...
.. image:: images/segment-settings.png
  :width: 600
  :alt: Screenshot showing Segment Settings Configuration in MailFly

* **Show warning if Segment hasn't been rebuilt for X hours** - Every time a :ref:`cron jobs<segment cron jobs>` runs, Segments are rebuilt. If there is an error that prevents a Segment from rebuilding, MailFly displays a warning message. This field allows you to configure the allowable length of time between rebuilds, after which the warning message appears.
...
.. image:: images/company-merge-settings.png
  :width: 600
  :alt: Screenshot showing Company Merge Settings Configuration in MailFly
...
.. image:: images/campaign-notification-settings.png
  :width: 600
  :alt: Screenshot showing Campaign Notification Settings Configuration in MailFly

.. image:: images/webhook-notification-settings.png
  :width: 600
  :alt: Screenshot showing Webhook Notification Settings Configuration in MailFly

If a Campaign or Webhook is automatically unpublished because of a high volume of errors, MailFly sends a notification alerting Users.
...
.. image:: images/landing-page-settings.png
  :width: 600
  :alt: Screenshot showing Landing Page Settings Configuration in MailFly
...
MailFly tracking settings
========================

.. image:: images/tracking-settings.png
  :width: 600
  :alt: Screenshot showing Tracking Settings Configuration in MailFly

* **Tracking code** - Insert this code on any page you would like to have tracked in MailFly before the ending ``</body>`` tag.
...
* **Identify visitor by tracking URL** - Select **Yes** to have MailFly begin tracking a Contact after the Contact clicks a link in an Email on a device where no cookie exists.
...
.. note:: 
  ...
  * Landing Pages including 4-byte UTF-8 characters, such as emojis and some Chinese or other non-Latin characters, in the Landing Page title or URL aren't tracked on a Contact's activity history in MailFly. All Latin characters used in English and other western languages are of 1-byte and are tracked.
...
.. image:: images/facebook-pixel-settings.png
  :width: 600
  :alt: Screenshot showing Facebook Pixel Settings Configuration in MailFly

* **Facebook Pixel ID** - Enter your Facebook Pixel ID and select the options you'd like to use the pixel for.

* **Enabled on your tracking landing page** - Select Yes to have MailFly append the Facebook Pixel to the MailFly tracking code to track Landing Pages where the tracking code exists.

* **Enabled on MailFly landing page** - Select Yes to have MailFly add the Facebook Pixel to MailFly landing pages.
...
.. image:: images/google-analytics-settings.png
  :width: 600
  :alt: Screenshot showing Google Analytics Settings Configuration in MailFly

* **Google Analytics ID** - Enter your Google Analytics ID and select the options you'd like to use the pixel for.

* **Enabled on your tracking landing page** - Select Yes to have MailFly append the Google Analytics script to the MailFly tracking code to track Landing Pages where the tracking code exists.

* **Enabled on MailFly landing page** - Select Yes to have MailFly add the Google Analytics script to MailFly landing pages.
...
.. image:: images/report-settings.png
  :width: 600
  :alt: Screenshot showing Report Settings Configuration in MailFly

* **Always quote data in CSV export** - Select Yes to wrap each MailFly field in double quotation marks when exported to a CSV file. For example: ``"First Name",”Last Name”,””, "some text"``.
...
.. image:: images/text-message-settings.png
  :width: 600
  :alt: Screenshot showing Text Message Settings Configuration in MailFly
...
Use a custom X.509 certificate and private key to secure communication between MailFly and the IDP. 
...
.. image:: images/webhook-settings.png
  :width: 600
  :alt: Screenshot showing Webhook Settings Configuration in MailFly
...
.. image:: images/social-settings.png
  :width: 600
  :alt: Screenshot showing Social Settings Configuration in MailFly

* **Twitter Handle Field** - This field stores the Twitter username for Users added to MailFly through Social Monitoring.
