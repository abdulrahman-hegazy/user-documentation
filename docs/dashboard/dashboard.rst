...
MailFly 1.4.0 introduced a customizable dashboard where each User can compose widgets with information they want to track. MailFly 2.0 brought a number of enhancements to the Dashboard.
...
- **Name:** describe what the widget displays. If not filled, MailFly will call it the same as the widget type you select.
...
MailFly installation come pre-loaded with 3 pre-defined dashboards. The one called default.json is imported automatically, when your dashboard doesn't contain any widgets. The other 2 predefined dashboards are provided as an example. You can export and import any other dashboards and switch between them. Pre-defined dashboards can be:

Previewed - This will display the dashboard widgets for preview. The dashboard will be loaded using your existing MailFly data. Nothing is saved or changed. Applied - This sets the dashboard as your primary dashboard. 
...
If a MailFly User doesn't have the 'see others' or 'see own' permissions for a bundle, they won't be able to create widgets for that bundle. However, the widget can still be visible on their dashboard. 
...
If a MailFly User has permission to see only their own data from a bundle, they will see only their own data in the Dashboard widgets. For example only Contacts which they own, Landing Page hits of the Landing Pages they created and so on.
